...
def convert_to_parquet():
    ...
    if input_folder:
            ...
            for root, dirs, files in os.walk(folder_path):
                for file in files:
                    if file.endswith(".csv"):
                        ...
                        output_file = os.path.join(output_dir, relative_path, f"{os.path.splitext(file)[0]}.parquet")
        ...
        for root, dirs, files in os.walk(input_folder):
            for dir in dirs:
                t = threading.Thread(target=convert_csv_to_parquet, args=(os.path.join(root, dir),))
                t.start()
                threads.append(t)
...
convert_button = tk.Button(root, text="Convert CSV to Parquet", command=convert_to_parquet, bg="#1DB954", fg="#FFFFFF", font=("Helvetica", 14, "bold"), relief="flat", activebackground="#1ED760")
convert_button.pack(pady=20)

# Create the result label
result_label = tk.Label(root, text="", fg="#FFFFFF", bg="#121212", font=("Helvetica", 14))
